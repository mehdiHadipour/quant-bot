...
class TestAdaptiveTrendDailyConfluence(unittest.TestCase):
    """Regression tests for the third change made in response to the same
    real-backtest finding (see TestAdaptiveTrendAdxGate's docstring): two
    rounds of single-filter ADX tweaks (1H, then 4H) didn't meaningfully
    improve the win rate (13.7% -> 13.3% -> 12.9%), so a genuine top-down
    multi-timeframe confluence check was added — the 1D trend (EMA50 vs
    EMA200) must agree with the 4H signal's own direction, or the signal
    is skipped. Fails open (does not block) when 1D history is
    insufficient, matching how other optional confirmations in this
    function already behave."""

    @staticmethod
    def _trending_frame(rng, n, slope, noise=0.6):
        close = 100 + np.arange(n) * slope + rng.normal(0, noise, n)
        return pd.DataFrame({
            "open": close - 0.1,
            "high": close + np.abs(rng.normal(0.5, 0.2, n)),
            "low": close - np.abs(rng.normal(0.5, 0.2, n)),
    ...
    def test_agreeing_daily_trend_allows_the_signal(self):
        rng = np.random.default_rng(21)
        n = 260
        df_up = self._trending_frame(rng, n, slope=0.8)
        # 1D bullish too (same upward slope) -> should agree with the BUY
        # direction the 4H crossover produces.
        df_1d = self._trending_frame(rng, n, slope=0.8)
        result = analyze_market(df_up, df_up, df_up, df_1d, "TESTUSDT")
        self.assertIsNotNone(result)
        self.assertEqual(result["direction"], "BUY")

    def test_contradicting_daily_trend_blocks_the_signal(self):
        rng = np.random.default_rng(23)
        n = 260
        # 4H (and 1H) cleanly bullish -> AdaptiveTrend direction is BUY.
        df_up = self._trending_frame(rng, n, slope=0.8)
        # 1D cleanly bearish -> contradicts the BUY direction.
        df_1d_down = self._trending_frame(rng, n, slope=-0.8)
        reasons = []
        result = analyze_market(df_up, df_up, df_up, df_1d_down, "TESTUSDT", reasons=reasons)
        self.assertIsNone(result)
        self.assertTrue(any("روند روزانه" in r for r in reasons))

    def test_insufficient_daily_data_does_not_block(self):
        """Fail-open: too little 1D history (get_timeframe_bias returns
        None) must not itself block an otherwise-valid signal — only an
        actively contradicting daily trend should."""
        rng = np.random.default_rng(25)
        n = 260
        df_up = self._trending_frame(rng, n, slope=0.8)
        df_1d_short = df_up.tail(10).reset_index(drop=True)  # far below MIN_CANDLES
        result = analyze_market(df_up, df_up, df_up, df_1d_short, "TESTUSDT")
        self.assertIsNotNone(result)
        self.assertEqual(result["direction"], "BUY")
...
class TestAdaptiveTrendAdxGate(unittest.TestCase):
    ...
    def test_uses_4h_adx_not_1h_adx(self):
        ...
        result = analyze_market(df_1h, df_1h, df_4h, df_4h, "TESTUSDT")
        self.assertIsNotNone(
            result,
