...
class TestCooldown(unittest.TestCase):
    ...
    def test_as_of_replays_against_historical_time_not_real_now(self):
        """Added for the backtest runner: a cooldown set for a historical
        date should be evaluated against that SAME historical timeline,
        not the real wall clock — otherwise every backtest cooldown would
        immediately look expired (historical dates are always in the
        past relative to real 'now')."""
        state = fresh_state()
        historical_now = datetime(2024, 1, 1, tzinfo=timezone.utc)
        trade_monitor.update_circuit_breaker(
            state, [{"symbol": "ETHUSDT", "result": "LOSS", "r_multiple": -1.0}], as_of=historical_now
        )
        # Immediately after, still within the historical cooldown window:
        self.assertTrue(trade_monitor.is_symbol_on_cooldown(
            state, "ETHUSDT", as_of=historical_now + timedelta(minutes=1)
        ))
        # Using the REAL current wall clock against a 2024 cooldown would
        # already show it as expired — confirms as_of is actually being
        # used, not silently falling back to real now.
        self.assertFalse(trade_monitor.is_symbol_on_cooldown(state, "ETHUSDT"))
...
class TestCircuitBreakerAndDashboard(unittest.TestCase):
    ...
    def test_check_open_trades_as_of_sets_historical_exit_time(self):
        """Same reasoning as the cooldown test above, for exit_time."""
        state = fresh_state()
        state["trades"] = [{"symbol": "BTCUSDT", "direction": "BUY", "entry": 100, "sl": 90, "tp": 120,
                             "status": "open", "initial_risk": 10}]
        historical_now = datetime(2024, 6, 15, tzinfo=timezone.utc)
