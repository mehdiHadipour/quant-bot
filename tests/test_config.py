...
class TestDirectionAllowed(unittest.TestCase):
    ...
    def test_unrestricted_symbol_allows_both(self):
        self.assertTrue(config.direction_allowed("DOGEUSDT", "BUY"))
        self.assertTrue(config.direction_allowed("DOGEUSDT", "SELL"))
    ...
    def test_default_sell_only_set_matches_the_requested_policy(self):
        expected = {"BNBUSDT", "SOLUSDT", "AVAXUSDT", "LINKUSDT", "NEARUSDT"}
    ...
    def test_bidirectional_symbols_are_in_symbols_but_not_sell_only(self):
        for sym in ("DOGEUSDT", "DOTUSDT", "ZECUSDT"):
            self.assertIn(sym, config.SYMBOLS)
            self.assertNotIn(sym, config.SELL_ONLY_SYMBOLS)
