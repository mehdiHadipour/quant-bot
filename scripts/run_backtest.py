...
KNOWN SIMPLIFICATIONS (v1 — read before trusting the numbers):
  - MAX_DAILY_LOSS_R is not enforced during the backtest (always passed
    as 0.0). Implementing it properly needs the exact same "today's
    already-realized loss" tracking main.py does live, across ALL
    symbols' interleaved history — a real feature, just not built yet.
...
def run_symbol_backtest(symbol, data_dir, verbose=False):
    ...
    for i in range(WARMUP_CANDLES, len(df_1h_all)):
        ...
        allowed, reason = can_open_trade(
            ...
            daily_loss_r=0.0,  # see KNOWN SIMPLIFICATIONS at the top of this file
