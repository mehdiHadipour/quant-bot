...
# Bounds every timeframe slice passed to analyze_market() to the same
# window size the live bot ever actually sees (data_engine.fetch_klines's
# default limit=300). Without this, run_backtest fed analyze_market the
# ENTIRE growing history at every tick (df.iloc[:idx+1] with idx climbing
# into the thousands) -- every indicator (EMA200, rolling ADX, etc.) was
# then recomputed over that whole, ever-larger series each time, turning
# a single symbol's walk into O(N^2) work. Observed in practice: a 90-day,
# 30-symbol backtest ran for 41+ minutes and was killed by the job
# timeout. Bounding to a fixed trailing window makes total work scale
# linearly with the number of ticks instead -- and is also more faithful
# to live trading, which never sees more than this many candles either.
...
def run_backtest(symbols, data_dir):
    ...
    for tick_i, tick_time in enumerate(all_ticks):
        for symbol in sorted(per_symbol_data.keys()):
            ...
            df_1h = df_1h_full.iloc[max(0, idx + 1 - LOOKBACK_CANDLES): idx + 1].reset_index(drop=True)
            ...
            df_4h = data["4h"][data["4h"]["open_time"] <= tick_time].tail(LOOKBACK_CANDLES).reset_index(drop=True)
            df_1d = data["1d"][data["1d"]["open_time"] <= tick_time].tail(LOOKBACK_CANDLES).reset_index(drop=True)
            df_15m = data["15m"][data["15m"]["open_time"] <= tick_time].tail(LOOKBACK_CANDLES).reset_index(drop=True)
