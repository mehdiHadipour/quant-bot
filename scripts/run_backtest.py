...
def run_portfolio_backtest(symbols, data_dir, verbose=False, max_abs_structure_score=None, invert_direction=False):
    ...
    invert_direction: EXPERIMENTAL, backtest-only (not wired into the
    live bot). Tests the "fade the bot" hypothesis properly instead of
    assuming it from a losing win rate: takes the OPPOSITE direction of
    every signal (BUY<->SELL) and rebuilds SL/TP from scratch for that
    flipped direction using the same ATR multipliers — this is NOT the
    same as "a SELL that lost automatically means an inverted BUY would
    have won": price may have chopped sideways and hit NEITHER target,
    or hit both in a wide candle, so the inverted trade needs its own
    real simulation through the exact same lifecycle (trailing,
    cooldown, risk gates) as any other trade, which is what this does."""
    all_frames = {}
    ...
    for as_of_ms in all_close_times[WARMUP_CANDLES:]:
        ...
        for symbol in symbols_with_data_this_tick:
            ...
            # V27.22: per-symbol direction policy — same single choke
            # point main.py's live loop uses (config.direction_allowed),
            # so backtest and live behavior stay matched.
            if not config.direction_allowed(symbol, direction):
                if verbose:
                    print(f"    [{as_of_dt.date()}] {symbol}: skipped — {direction} not allowed for this symbol")
                continue
            ...
            group_symbols = (
                ...
                if (config.DIVERSIFICATION_ENABLED and symbol in config.COMMODITY_SYMBOLS)
                else None
            )
            allowed, reason = can_open_trade(
                ...
                max_open_per_group=config.MAX_OPEN_PER_MARKET_GROUP,
            )
            if not allowed:
                if verbose:
...
def main():
    ...
    parser.add_argument(
        "--invert-direction", action="store_true",
        help="EXPERIMENTAL: take the OPPOSITE of every signal's direction "
             "(BUY<->SELL), rebuilding SL/TP from scratch for the flipped "
             "direction, and simulate that instead. Tests a 'fade the bot' "
             "hypothesis properly (a losing SELL does NOT automatically "
             "mean an inverted BUY would have won) instead of assuming it.",
    )
    ...
    if args.invert_direction:
        print("(\u0622\u0632\u0645\u0627\u06cc\u0634\u06cc: \u062c\u0647\u062a \u0647\u0631 \u0633\u06cc\u06af\u0646\u0627\u0644 \u0645\u0639\u06a9\u0648\u0633 \u0634\u062f\u0647 \u0627\u0633\u062a \u2014 \u0641\u0631\u0636\u06cc\u0647\u0654 Fade)")
