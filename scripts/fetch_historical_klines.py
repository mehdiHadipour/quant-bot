...
from weex_data_engine import _extract_rows as _extract_weex_rows  # noqa: E402
...
def _fetch_page_weex(symbol, interval, end_time_ms, limit=1000):
    ...
    try:
        ...
        if resp.status_code != 200:
            print(f"  [WEEX diag] {symbol} {interval}: HTTP {resp.status_code}: {resp.text[:200]!r}")
            return None
        try:
            payload = resp.json()
        except ValueError:
            print(f"  [WEEX diag] {symbol} {interval}: 200 but non-JSON body: {resp.text[:200]!r}")
            return None
        rows = _extract_weex_rows(payload)
        if rows is None:
            print(f"  [WEEX diag] {symbol} {interval}: 200 but unrecognized shape: {str(payload)[:200]!r}")
            return None
        return rows
    except requests.RequestException as e:
        print(f"  [WEEX diag] {symbol} {interval}: request error: {e}")
        return None
