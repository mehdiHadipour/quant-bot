...
def run(data_dir, symbols, fee):
    ...
    return {
      "⚠️_WARNING": (
          "This report is from a SEPARATE, simplified continuous-portfolio-weight "
          "research model (adaptive_strategy.signal_and_weight), NOT the discrete "
          "SL/TP trades the live bot (main.py) actually places and alerts on via "
          "Telegram. These numbers do not describe live bot behavior — see "
          "backtest_results.csv (from scripts/run_backtest.py) for that instead."
      ),
      "cost_round_trip":fee,
...
if __name__=="__main__":
    ...
    ap.add_argument("--symbols",default="BNBUSDT,SOLUSDT,DOGEUSDT,AVAXUSDT,LINKUSDT,DOTUSDT,ZECUSDT,NEARUSDT")
