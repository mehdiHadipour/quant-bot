#!/usr/bin/env python3
"""
🚀 AI Quant Bot v25.10 - Critical Fix: history.csv Corruption + Cron Jitter Note
...
def _read_history_csv():
    """Read data/history.csv, self-healing a schema mismatch left over
    from before v25.10 (see HISTORY_COLUMNS above). Tries a normal read
    first; on a column-count parser error, falls back to a tolerant read
    that drops only the specific malformed row(s), reindexes everything
    to the current fixed schema, and rewrites the file once — so this
    heals permanently instead of warning every single cycle forever."""
    if not os.path.exists(HISTORY_FILE):
        return None
    try:
        return pd.read_csv(HISTORY_FILE)
    except Exception as e:
        log.warning(f"history.csv could not be parsed normally ({e}); attempting a tolerant repair...")
        try:
            df = pd.read_csv(HISTORY_FILE, on_bad_lines="skip", engine="python")
            df = df.reindex(columns=HISTORY_COLUMNS)
            df.to_csv(HISTORY_FILE, index=False)
            log.info(f"✅ history.csv repaired: {len(df)} row(s) kept under the current fixed schema.")
            return df
        except Exception as e2:
            log.warning(f"Could not repair history.csv, leaving it untouched: {e2}")
            return None


def append_to_history(trade):
    """Append a closed trade to the history CSV, always using the fixed
    HISTORY_COLUMNS schema — never whatever raw keys happen to be on
    `trade` this cycle (see HISTORY_COLUMNS above for why)."""
    ...
    # If the existing file's header doesn't already match the current
    # fixed schema (e.g. it was written by a pre-v25.10 version, or is
    # the corrupted mixed-schema state that caused the original bug),
    # repair/migrate the whole file first so this new row lines up with
    # everything else instead of creating yet another mismatch.
    try:
        existing_header = pd.read_csv(HISTORY_FILE, nrows=0)
        if list(existing_header.columns) != HISTORY_COLUMNS:
            raise ValueError("history.csv schema is out of date")
        new_row_df.to_csv(HISTORY_FILE, mode="a", header=False, index=False)
    except Exception:
        repaired = _read_history_csv()
        combined = (
            pd.concat([repaired, new_row_df], ignore_index=True)
            if repaired is not None else new_row_df
        )
