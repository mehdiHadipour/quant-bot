...
from config import (
    ...
    BUY_ONLY_SYMBOLS, SELL_ONLY_SYMBOLS,
)
from data_engine import fetch_klines, fetch_funding_rate
from weex_data_engine import fetch_klines as fetch_weex_klines, fetch_funding_rate as fetch_weex_funding_rate
from asset_universe import live_tradfi_symbols, is_tradfi_symbol, tradfi_candidates
...
HISTORY_COLUMNS = [
    ...
    "session", "fib_ote", "hli", "order_flow_bias", "order_flow_strength",
    "volume_profile_context", "diagnostic_snapshot",
]
...
def fetch_all_klines(symbols):
    """Fetch market data from Binance for crypto and WEEX V3 for approved TradFi.

    TradFi symbols are never fetched for live trading unless they are explicitly
    approved by the out-of-sample promotion policy.
    """
    jobs = {}
    results = {symbol: {interval: None for interval in INTERVALS} for symbol in symbols}
    tradfi = live_tradfi_symbols()
    with ThreadPoolExecutor(max_workers=min(12, len(symbols) * len(INTERVALS) or 1)) as executor:
        for symbol in symbols:
            is_weex = symbol in tradfi
            for interval in INTERVALS:
                fn = fetch_weex_klines if is_weex else fetch_klines
                jobs[executor.submit(fn, symbol, interval)] = (symbol, interval, is_weex)
        for future in as_completed(jobs):
            symbol, interval, is_weex = jobs[future]
            try:
                results[symbol][interval] = future.result()
            except Exception as e:
                log.error(f"Error fetching {symbol} {interval} ({'WEEX' if is_weex else 'crypto'}): {e}")
...
def process_symbol(state, symbol, klines_for_symbol, counters):
    ...
    # TradFi is research-first: only explicitly promoted symbols can enter live flow.
    if is_tradfi_symbol(symbol) and symbol not in live_tradfi_symbols():
        log.info(f"🧪 {symbol}: TradFi candidate است ولی هنوز OOS approval ندارد؛ فقط research/backtest مجاز است.")
        return
    ...
    if is_tradfi_symbol(symbol) and symbol in live_tradfi_symbols():
        funding_rate, funding_source, funding_fail_reason = fetch_weex_funding_rate(symbol)
    else:
        funding_rate, funding_source, funding_fail_reason = fetch_funding_rate(symbol)

    diagnostics = result.get("diagnostics", {})
    vp_diag = diagnostics.get("volume_profile", {}) or {}
    ichi_diag = result.get("ichimoku", {}) or {}
    msg += (
        f"📚 VP: POC {vp_diag.get('poc', 0) or 0:,.2f} | VAH {vp_diag.get('vah', 0) or 0:,.2f} | VAL {vp_diag.get('val', 0) or 0:,.2f} | {vp_diag.get('context', 'N/A')}\n"
        f"📐 MACD: {'هم‌جهت' if diagnostics.get('macd_aligned') else 'غیرهم‌جهت'} | "
        f"☁️ Ichimoku: 4H {'BULL' if ichi_diag.get('4h_bull') else 'BEAR' if ichi_diag.get('4h_bear') else 'NEUTRAL'} / "
        f"1D {'BULL' if ichi_diag.get('1d_bull') else 'BEAR' if ichi_diag.get('1d_bear') else 'NEUTRAL'}\n"
        f"🎯 OTE Gate: {'PASS' if diagnostics.get('fibonacci_ote') else 'FAIL'} | "
        f"OrderFlow: {diagnostics.get('order_flow_bias', 'NEUTRAL')} ({float(diagnostics.get('order_flow_strength', 0) or 0):.2f})\n"
    )
    ...
    if not result:
        ...
        # Repeated gate failures are useful diagnostics: alert only after a
        # threshold so Telegram is not flooded by normal no-signal cycles.
        key = f"{symbol}|{reason[:140]}"
        skips = state.setdefault("gate_skip_counts", {})
        alerted = state.setdefault("gate_skip_alerted", {})
        skips[key] = int(skips.get(key, 0)) + 1
        if skips[key] == 5 and not alerted.get(key):
            send_telegram_alert(
                f"⚠️ <b>Signal Gate Sentinel</b>\n"
                f"نماد: {symbol}\n"
                f"گیت تکراراً سیگنال را رد کرده است: {reason}\n"
                f"تعداد تکرار: {skips[key]}\n"
                f"این هشدار تشخیصی است و به معنی خرابی قطعی ربات نیست؛ برای بررسی ضعف داده/المان ارسال شده است."
            )
...
"""
    ...
    msg += (
        f"\n🧬 <b>Fibonacci OTE: {fib.get('zone', 'NONE')}</b> | الزام ورود: {'YES' if fib.get('required_ote') else 'NO'}\n"
        f"   Swing High: {fib.get('swing_high', 0) or 0:,.2f} | Swing Low: {fib.get('swing_low', 0) or 0:,.2f}\n"
        f"   61.8%: {fib.get('f618', 0) or 0:,.2f} | 78.6%: {fib.get('f786', 0) or 0:,.2f} | Lookback: {fib.get('lookback', 72)}\n"
    )
    ...
    if smart:
        ...
        msg += (
            ...
            f"HLI {smart.get('hyper_liquidity', {}).get('score', 0.0):.0f}\n"
        )
    ...
    state["trades"].append({
        ...
        "session": smart.get("session", {}).get("name", "UNKNOWN"),
        "fib_ote": bool(result.get("fibonacci", {}).get("ote", False)),
        "hli": float(smart.get("hyper_liquidity", {}).get("score", 0) or 0),
        "order_flow_bias": smart.get("footprint", {}).get("bias", "NEUTRAL"),
        "order_flow_strength": float(smart.get("footprint", {}).get("strength", 0) or 0),
        "volume_profile_context": smart.get("volume_profile", {}).get("context", "UNKNOWN"),
        "diagnostic_snapshot": {
            "fibonacci_ote": bool(result.get("fibonacci", {}).get("ote", False)),
            "liquidity_sweep": result.get("liquidity_sweep"),
            "order_flow_bias": smart.get("footprint", {}).get("bias", "NEUTRAL"),
            "order_flow_strength": float(smart.get("footprint", {}).get("strength", 0) or 0),
            "volume_profile_ok": bool(smart.get("volume_profile", {}).get("ok", False)),
            "volume_profile_context": smart.get("volume_profile", {}).get("context", "UNKNOWN"),
            "hyper_liquidity": float(smart.get("hyper_liquidity", {}).get("score", 0) or 0),
            "session": smart.get("session", {}).get("name", "UNKNOWN"),
            "fundamental_score": float(smart.get("fundamental_score", 0) or 0),
        },
    })
    save_state(state)
...
def main():
    ...
    tradfi_live = sorted(live_tradfi_symbols())
    active_symbols = list(dict.fromkeys(SYMBOLS + tradfi_live))
    if tradfi_live:
        log.info("🟢 Approved WEEX TradFi symbols: %s", ", ".join(tradfi_live))
    else:
        log.info("🧪 No WEEX TradFi symbol is live-approved yet; research candidates remain isolated.")
    klines = fetch_all_klines(active_symbols)
    ...
    check_fetch_health(state, counters["fetched_ok"], len(active_symbols))

    # Performance Sentinel: inspect recently closed history and warn when a
    # symbol/direction/session has demonstrably negative expectancy. This is
    # diagnostic only; the promotion script is the authority for TradFi live approval.
    try:
        hist = _read_history_csv()
        if hist is not None and not hist.empty:
            diag = diagnose(hist.to_dict("records"))
            if diag.get("alerts"):
                lines = ["🚨 <b>Performance Sentinel</b>", "عامل بالقوه ضعف سیگنال شناسایی شد:"]
                for a in diag["alerts"][:8]:
                    scope = a.get("scope", "")
                    name = a.get("symbol") or a.get("name") or "UNKNOWN"
                    direction = a.get("direction", "ALL")
                    lines.append(f"• {scope} | {name} | {direction} | {a.get('reason')} | trades={a.get('trades')} | NetR={a.get('net_r',0):+.2f} | PF={a.get('pf',0):.2f}")
                send_telegram_alert("\n".join(lines))
                log.warning("Performance Sentinel alerts: %s", diag["alerts"])
    except Exception as exc:
        log.exception("Performance Sentinel failed safely: %s", exc)
    state["last_cycle_completed_at"] = datetime.now(timezone.utc).isoformat()
    save_state(state)
    ...
    if counters["funding_ok"] == 0 and len(active_symbols) > 0:
