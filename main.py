...
"""
    ...
    if ichimoku:
        cloud_fa = {"above": "بالای ابر ☁️⬆️", "below": "زیر ابر ☁️⬇️", "inside": "داخل ابر (خنثی)"}
        cross_fa = {"bullish": "صعودی", "bearish": "نزولی", "neutral": "خنثی"}
        color_fa = {"bullish": "سبز", "bearish": "قرمز"}
        chikou_fa = {"bullish": "تأیید صعودی", "bearish": "تأیید نزولی", "neutral": "خنثی"}
        msg += (
            f"☁️ Ichimoku: قیمت {cloud_fa.get(ichimoku['price_vs_cloud'], '?')} | "
            f"تنکان/کیجون {cross_fa.get(ichimoku['tenkan_kijun'], '?')} | "
            f"ابر {color_fa.get(ichimoku['cloud_color'], '?')} | "
            f"چیکو {chikou_fa.get(ichimoku['chikou'], '?')} | "
            f"امتیاز {ichimoku['score']:+d}\n"
        )
