...
from config import (
    ...
    NEAR_TP_LOCK_TRIGGER_R, NEAR_TP_LOCK_R,
...
def process_symbol(state, symbol, klines_for_symbol, counters):
    ...
    if trailing_moves:
        for move in trailing_moves:
            ...
            if move["stage"] == "near_tp_lock":
                send_telegram_alert(
                    f"🔒 <b>نزدیک TP — سود بیشتری قفل شد (Near-TP Lock)</b> - {symbol}\n"
                    ...
                    f"معامله بیش از {NEAR_TP_LOCK_TRIGGER_R:.0%} مسیر تا TP را طی کرده — از این لحظه، حتی اگر "
                    f"بازار کامل برگردد، این معامله حداقل {NEAR_TP_LOCK_R:.2f}R سود واقعی برایتان تضمین کرده."
                )
            elif move["stage"] == "partial_lock":
    ...
    # V27.22: per-symbol direction policy, based on the user's own
    # real (demo-account) trade results — not an independent backtest
    # finding. See config.SELL_ONLY_SYMBOLS/direction_allowed for the
    # single choke point this shares with scripts/run_backtest.py, so
    # backtest and live behavior stay matched.
    if not direction_allowed(symbol, direction):
        log.info(f"🔍 {symbol}: بدون سیگنال — جهت {direction} برای این نماد غیرمجاز است (فقط SELL مجاز است)")
        return
