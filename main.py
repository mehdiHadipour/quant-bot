...
Adds: (1) high/low-based TP/SL/trailing/warning detection instead of
close-only, fixing missed intrabar touches that previously left trades
"open" forever with no closing message; (2) wick/shadow rejection
scoring; (3) taker buy/sell order-flow imbalance scoring (real data from
Binance's kline response, not fabricated). All earlier features
unchanged: v22 real automated trailing stop, 5-minute check interval,
v21 state-persistence fix, v20 SELL-signal-symmetry fix, multi-timeframe
confluence, R-multiple dashboard, structured logging, self-healing
state, consecutive-fetch-failure alerting, ADX/Bollinger/Stochastic +
RSI divergence warnings, per-symbol cooldown, 10-symbol coverage,
parallel fetch, daily reports.
"""
