#!/usr/bin/env python3
"""
🚀 AI Quant Bot v28.0 - Two-Stage Trailing Stop (Lock Partial Profit) + 10-Minute Cycle
...
def process_symbol(state, symbol, klines_for_symbol, counters):
    ...
    if not (atr > 0 and price > 0):
        log.warning(f"🛡️ {symbol}: ATR/price invalid; refusing to create a signal.")
        return
...
📈 BUY: {result['buy']:.1f}% | 📉 SELL: {result['sell']:.1f}% | 🎯 اعتماد: {result.get('confidence', 0):.1f}%
🧭 رژیم بازار: 4H={result.get('regime_4h', '—')} | 1D={result.get('regime_1d', '—')}
