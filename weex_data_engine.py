...
def _extract_rows(payload):
    """WEEX's response could plausibly be a bare array (like Binance) or an
    enveloped {"code":..., "data": [...]} object (common on many exchange
    APIs) -- this was never independently verified against the live API
    from this environment, so handle both defensively instead of assuming
    one and silently mis-parsing (or crashing on) the other.
    """
        ...
        try:
            ...
            if resp.status_code == 200:
                try:
                    ...
                    log.warning(f"[{tag}] WEEX 200 but non-JSON body: {resp.text[:300]!r}")
                    return None
                rows = _extract_rows(payload)
                if rows is None:
                    log.warning(f"[{tag}] WEEX 200 but unrecognized response shape: {str(payload)[:300]!r}")
                    return None
                if rows:
                    return pd.DataFrame([{
                        "open_time": int(x[0]),
                ...
                log.warning(f"[{tag}] WEEX returned an empty kline list -- symbol may not exist "
                            f"on WEEX or this interval isn't supported for it")
                return None
            elif resp.status_code in (403, 429):
                log.warning(f"[{tag}] WEEX returned {resp.status_code}: {resp.text[:300]!r} -- backing off...")
                time.sleep(2)
            else:
                log.warning(f"[{tag}] WEEX returned status {resp.status_code}: {resp.text[:300]!r}")
                time.sleep(1)
