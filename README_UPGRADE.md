...
## v27.9.1 — Fix Lint Failure in v27.9's New Test File

`tests/test_run_backtest.py` (added in v27.9) had a leftover unused
variable (`total_ever_opened`) from an earlier draft, plus two more
unused-capture cases (`closed`/`per_symbol` not read after being
assigned in two tests) that the same lint rule would have caught next.
Removed the dead line and switched to not capturing (or `_`-capturing)
return values that aren't actually used. Verified with a manual AST-
based scan across this file AND every other recently-touched core file
(main.py, trade_monitor.py, indicators.py, risk_engine.py, both backtest
scripts) for the same pattern — none found. All 4 portfolio-gate tests
re-run and still pass; full suite still 91/92 (same pre-existing,
unrelated fake-`ta`-stub limitation as every prior version).

## v27.9.2 — Root-Caused the "100% SELL" Pattern With Real Data + TP/SL A/B Testing

Analyzed the actual `backtest_results.csv` from a real portfolio-backtest
run (226 trades, BTC/ETH/SOL, Nov 2025-Jul 2026):

**The 100% SELL pattern is NOT a bug.** Verified the price data directly:
BTC -41.2%, ETH -50.4%, SOL -60.0% over the tested window — a genuine,
severe, sustained bear market. `market_bias` (the hard gate on allowed
direction) is correctly derived from 4H EMA50/EMA200; a bull-market BUY
gate simply never opened because there wasn't a sustained 4H uptrend to
detect. (Separately confirmed the per-trade "Trend (EMA)=+20" values
seen in some SELL trades are NOT a contradiction — that's the 1H-
timeframe trend score, one of many weighted scoring inputs, correctly
disagreeing with the 4H/1D gate that actually determines direction —
a real, expected short-term-pullback-within-a-larger-downtrend signal,
not a bug.)

**The real, evidence-based finding — r_multiple distribution:**
```
Full TP wins (+1.667R):     26 trades (11.5%)
Partial-lock exits (+0.5R): 50 trades (22.1%)
Breakeven exits (0.0R):     57 trades (25.2%)
Full SL losses (-1.0R):     93 trades (41.2%)
```
41.2% of trades never got ANY relief before hitting full SL. Of the
58.8% that DID reach at least the trailing trigger (halfway to TP),
only 26/133 (19.5%) ever reached full TP — the rest reversed and gave
back the move. This points at TP/SL calibration and entry timing in a
high-volatility bear market, not direction-detection — a much more
specific, actionable diagnosis than "the strategy doesn't work."

**Added: A/B testing TP/SL multipliers with zero code changes.**
`.github/workflows/backtest.yml` now exposes `atr_sl_multiplier` and
`atr_tp_multiplier` as workflow inputs (defaults match the live bot:
1.8 / 3.0), passed through as env vars that `config.py` already reads.
This lets the user directly test hypotheses like "a closer TP would
raise the hit rate enough to offset the smaller reward" from the
Actions UI — run the same backtest multiple times with different
multiplier combinations, compare Expectancy each time — without
touching any file. This is the responsible next step: test parameter
changes empirically before ever changing the live bot's defaults.

## v27.10 — Testable Extremity-Guard Hypothesis (Backtest-Only, Not Live)

Turned the finding from analyzing a real backtest run into an actual
testable experiment, rather than "copying" an unverifiable claim about
some other bot: trades where |Structure/Breakout| was unusually large
were far more likely to reverse before reaching full TP (mean -4.9 to
-5.9 for breakeven/partial-lock exits vs -2.8 to -2.9 for full
wins/losses) — consistent with an extreme structure reading marking an
already-extended/climactic move rather than the start of one.

Added `--max-abs-structure-score` to `scripts/run_backtest.py`
(EXPERIMENTAL, backtest-only — NOT wired into indicators.py or the live
bot): if set, any signal whose |Structure/Breakout| score exceeds it is
rejected entirely. Also exposed as a `max_abs_structure_score` input on
`.github/workflows/backtest.yml`, leave empty to disable. Verified with
2 new tests: the filter correctly rejects an extreme-score signal while
leaving a normal run unaffected (5/5 backtest tests pass).

This is deliberately still just a hypothesis-testing tool, not a
conclusion — try a few threshold values (e.g. 20, 25, 30) against the
same historical data and compare Expectancy/Profit Factor before
considering porting anything into the live signal logic.

## v27.11 — Testable "Fade the Bot" Hypothesis (Backtest-Only, Not Live)

Requested after noticing the live bot's SELL signals have been losing
(per the real backtest data already analyzed). Turned "trade the
opposite of the bot" into a properly testable experiment instead of
trusting a few days of manual anecdotal trading — a losing win rate
doesn't automatically mean the inverse wins: price may chop and hit
neither target, or hit both in one wide candle, so a flipped trade needs
its own real simulation, not an assumption.

Added `--invert-direction` to `scripts/run_backtest.py` (EXPERIMENTAL,
backtest-only — NOT wired into indicators.py or the live bot): every
signal's direction is flipped (BUY<->SELL) and SL/TP are rebuilt from
scratch for the new direction using the same ATR multipliers, then run
through the exact same lifecycle (trailing, cooldown, risk gates) as
any other trade. Also exposed as an `invert_direction` input on
`.github/workflows/backtest.yml`. Verified with a new test: direction
correctly flips and the inverted trade's SL/TP land on the correct
sides of entry for the new direction (not just mirrored blindly) —
6/6 backtest tests pass.

Still just a hypothesis-testing tool, not a conclusion. Run it once
with `invert_direction=true` against the same historical window
already tested, and compare Expectancy/win-rate/Profit Factor honestly
against both the normal-direction result and a coin-flip baseline
before treating "trade the opposite" as validated.
