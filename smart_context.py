...
def session_context(df):
    """London/NY/Asia. DST-aware local clocks; only configured sessions are allowed."""
    if df is None or df.empty:
    ...
    try:
        ...
        london=8 <= l.hour < 17
        ny=8 <= n.hour < 17
        asia = ASIA_ENABLED.strip().lower() not in {"0", "false", "no", "off"} and 9 <= t.hour < 18
        if london and ny:
            return {"name":"LONDON_NY_OVERLAP","quality":float(SESSION_WEIGHTS.get("LONDON_NY_OVERLAP", 1.0)),"allow":True}
        if london and asia:
            return {"name":"ASIA_EUROPE_OVERLAP","quality":float(SESSION_WEIGHTS.get("ASIA_EUROPE_OVERLAP", 0.86)),"allow":True}
        if london:
            return {"name":"LONDON","quality":float(SESSION_WEIGHTS.get("LONDON", 0.88)),"allow":True}
        if ny:
            return {"name":"NEW_YORK","quality":float(SESSION_WEIGHTS.get("NEW_YORK", 0.92)),"allow":True}
        if asia:
            return {"name":"ASIA","quality":float(SESSION_WEIGHTS.get("ASIA", 0.72)),"allow":True}
        return {"name":"OFF_SESSION","quality":0.0,"allow":False}
    except (TypeError,ValueError,OSError,OverflowError):
        return {"name":"OFF_SESSION","quality":0.0,"allow":False}
...
def evaluate(symbol, direction, df_1h, *, min_session_quality=0.55, asset_class="CRYPTO"):
    ...
    if symbol.upper() in STRICT_SYMBOLS:
    ...
    if sess["name"] in NEGATIVE_SESSIONS:
