...
from config import ASIA_ENABLED, SESSION_WEIGHTS, STRICT_SYMBOLS, STRICT_MIN_HLI, STRICT_MIN_SCORE, NEGATIVE_SESSIONS, NEGATIVE_SESSION_MIN_HLI, NEGATIVE_SESSION_MIN_SCORE, FIB_LOOKBACK, FIB_OTE_LOW, FIB_OTE_HIGH
from volume_profile import context as vp_context

def macd(df, fast=12, slow=26, signal=9):
 emaf=df.close.ewm(span=fast,adjust=False,min_periods=fast).mean(); emas=df.close.ewm(span=slow,adjust=False,min_periods=slow).mean(); line=emaf-emas; sig=line.ewm(span=signal,adjust=False,min_periods=signal).mean(); return line,sig,line-sig
...
def run(df, ichi=(9,26,52), atr_n=14, disp_min=.85, score_thr=65, fib_lookback=FIB_LOOKBACK, fib_ote_lo=FIB_OTE_LOW, fib_ote_hi=FIB_OTE_HIGH, macd_gate=False, strict=False, vp_lookback=60, vp_bins=40, asset_class='CRYPTO'):
 ...
 ph=df.high.shift(1).rolling(20).max(); pl=df.low.shift(1).rolling(20).min()
 bull_sweep=(df.low<pl)&(df.close>pl); bear_sweep=(df.high>ph)&(df.close<ph)
 mss_up=df.close > df.high.shift(1).rolling(8).max(); mss_dn=df.close < df.low.shift(1).rolling(8).min()
 ...
 prior_b=bull_sweep.shift(1).rolling(4).max().fillna(False).astype(bool); prior_s=bear_sweep.shift(1).rolling(4).max().fillna(False).astype(bool)
 ...
 mid40=(df.high.shift(1).rolling(40).max()+df.low.shift(1).rolling(40).min())/2
 ...
 fib_score_buy=10*fib_buy_ote.astype(int)+4*fib_buy_mid.astype(int)
 fib_score_sell=10*fib_sell_ote.astype(int)+4*fib_sell_mid.astype(int)
 session_bonus=np.select([sess==SESSION_WEIGHTS.get('LONDON_NY_OVERLAP',1.0),sess==SESSION_WEIGHTS.get('ASIA_EUROPE_OVERLAP',.86),sess==SESSION_WEIGHTS.get('LONDON',.88),sess==SESSION_WEIGHTS.get('NEW_YORK',.92),sess==SESSION_WEIGHTS.get('ASIA',.72)],[5.0,4.0,3.0,2.0,0.0],default=0.0)
 score_orderflow = 0
 ...
 sess_thr=np.select([sess==SESSION_WEIGHTS.get('LONDON_NY_OVERLAP',1.0),sess==SESSION_WEIGHTS.get('ASIA_EUROPE_OVERLAP',.86),sess==SESSION_WEIGHTS.get('LONDON',.88),sess==SESSION_WEIGHTS.get('NEW_YORK',.92),sess==SESSION_WEIGHTS.get('ASIA',.72)],[70.0,68.0,65.0,74.0,78.0],default=999.0)
 ...
 if os.getenv('REQUIRE_FIB_OTE','1').strip().lower() not in {'0','false','no','off'}:
 ...
 for i in np.where(c_buy|c_sell)[0]:
  ...
  if direction=='BUY': ext=float(df.low.iloc[sw]); sl=ext-.10*aa; opp=float(df.high.iloc[i-40:i].max()); tp=max(opp,entry+2*abs(entry-sl)); tp=min(tp,entry+4*abs(entry-sl))
  else: ext=float(df.high.iloc[sw]); sl=ext+.10*aa; opp=float(df.low.iloc[i-40:i].min()); tp=min(opp,entry-2*abs(entry-sl)); tp=max(tp,entry-4*abs(entry-sl))
  risk=abs(entry-sl)
  if risk<=0: continue
  rr=abs(tp-entry)/risk
  if rr<1.8: continue
  vp=vp_context(df,i,lookback=vp_lookback,bins=vp_bins)
 ...
 for z in rows:
  ...
  entry_i=z['idx']; end=min(len(df),entry_i+97); mfe=mae=0.; initial_adverse=False; ex=xp=None; status=None
