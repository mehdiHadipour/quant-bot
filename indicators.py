...
from config import MIN_SIGNAL_PROBABILITY, MIN_ADX, MIN_ATR_PERCENT, SMART_CONTEXT_ENABLED, MIN_SESSION_QUALITY, ICHIMOKU_ENABLED
...
# --- Ichimoku Kinko Hyo periods (the traditional 9/26/52 settings) ---
...
def compute_ichimoku(df):
    """Standard Ichimoku Kinko Hyo (9/26/52), computed by hand rather than
    via a library's built-in indicator so the Senkou-span forward-shift
    semantics are unambiguous, auditable, and independently testable.

    The Senkou (leading) spans are traditionally PLOTTED 26 candles AHEAD
    of the data used to compute them -- so "today's cloud" is the raw
    span value computed from data as of 26 candles ago, not today's raw
    value (that value is the cloud that will be plotted 26 candles into
    the future, past the current candle -- a common source of off-by-one
    bugs in hand-rolled Ichimoku code). Chikou (lagging) span is simply
    today's close, conventionally compared against price from 26 candles
    ago -- the point it visually overlaps on the chart.

    Returns None if there isn't yet enough history (needs
    ICHIMOKU_SENKOU_B_WINDOW + ICHIMOKU_KIJUN_WINDOW = 78 candles: 52 for
    the slowest line, plus another 26 so "today's" already-plotted cloud
    exists) or if any required value is NaN -- callers should treat that
    as "no Ichimoku signal this cycle" (weight 0), never as a reason to
    block an otherwise-valid signal, matching how every other optional
    signal in this file fails soft.

    Otherwise returns a dict with the four classic Ichimoku reads used
    here, each independently interpretable, plus a combined weighted
    "score" (range -40..+40, comparable in magnitude to trend_score/
    momentum_score) for analyze_market() to fold into total_score:
      - "price_vs_cloud": "above" | "below" | "inside"      (+-15 | 0)
      - "tenkan_kijun":   "bullish" | "bearish" | "neutral" (+-10 | 0)
      - "cloud_color":    "bullish" | "bearish"              (+-5)
      - "chikou":         "bullish" | "bearish" | "neutral" (+-10 | 0)
    """
    min_needed = ICHIMOKU_SENKOU_B_WINDOW + ICHIMOKU_KIJUN_WINDOW
    if len(df) < min_needed:
        return None

    high, low, close = df['high'], df['low'], df['close']
    tenkan = (high.rolling(ICHIMOKU_TENKAN_WINDOW).max() + low.rolling(ICHIMOKU_TENKAN_WINDOW).min()) / 2
    kijun = (high.rolling(ICHIMOKU_KIJUN_WINDOW).max() + low.rolling(ICHIMOKU_KIJUN_WINDOW).min()) / 2
    senkou_a_raw = (tenkan + kijun) / 2
    senkou_b_raw = (high.rolling(ICHIMOKU_SENKOU_B_WINDOW).max() + low.rolling(ICHIMOKU_SENKOU_B_WINDOW).min()) / 2

    idx_cloud = -1 - ICHIMOKU_KIJUN_WINDOW  # "today's" plotted cloud = the raw span from 26 candles ago
    cloud_a_now = senkou_a_raw.iloc[idx_cloud]
    cloud_b_now = senkou_b_raw.iloc[idx_cloud]
    tenkan_now = tenkan.iloc[-1]
    kijun_now = kijun.iloc[-1]
    close_now = close.iloc[-1]
    close_26_ago = close.iloc[-1 - ICHIMOKU_KIJUN_WINDOW]

    if any(pd.isna(v) for v in [cloud_a_now, cloud_b_now, tenkan_now, kijun_now, close_now, close_26_ago]):
        return None
    ...
    if close_now > cloud_top:
        price_vs_cloud, cloud_component = "above", 15
    elif close_now < cloud_bottom:
        price_vs_cloud, cloud_component = "below", -15
    else:
        price_vs_cloud, cloud_component = "inside", 0

    if tenkan_now > kijun_now:
        tenkan_kijun, cross_component = "bullish", 10
    elif tenkan_now < kijun_now:
        tenkan_kijun, cross_component = "bearish", -10
    else:
        tenkan_kijun, cross_component = "neutral", 0

    cloud_color = "bullish" if cloud_a_now > cloud_b_now else "bearish"
    color_component = 5 if cloud_color == "bullish" else -5

    if close_now > close_26_ago:
        chikou, chikou_component = "bullish", 10
    elif close_now < close_26_ago:
        chikou, chikou_component = "bearish", -10
    else:
        chikou, chikou_component = "neutral", 0
    ...
    return {
        ...
        "chikou": chikou,
        "score": cloud_component + cross_component + color_component + chikou_component,
        "tenkan": tenkan_now,
        "kijun": kijun_now,
        "cloud_top": cloud_top,
...
def analyze_market(df_15m, df_1h, df_4h, df_1d, symbol, funding_rate=None, reasons=None):
    ...
    # --- Ichimoku Kinko Hyo (9/26/52): cloud position, Tenkan/Kijun cross,
    # cloud color, and Chikou-span confirmation, folded in as one more
    # weighted component (same treatment as trend/momentum/etc, not a hard
    # gate). See compute_ichimoku()'s docstring for the exact scoring and
    # why the forward-shift matters. Fails soft to 0 (no Ichimoku opinion
    # this cycle) whenever ICHIMOKU_ENABLED is off or there isn't yet
    # enough history -- never blocks an otherwise-valid signal on its own.
    ichimoku = compute_ichimoku(df) if ICHIMOKU_ENABLED else None
    ichimoku_score = ichimoku["score"] if ichimoku else 0
    ...
    return {"direction": direction, "buy": buy_prob, "sell": sell_prob, "neutral": neutral_prob, "atr": atr, "price": current_close, "symbol": symbol, "adx": adx, "divergence": divergence, "buy_ratio": buy_ratio, "fvg": fvg, "vwap": vwap, "funding_rate": funding_rate, "liquidity_sweep": liquidity_sweep, "smart_context": smart, "ichimoku": ichimoku}
