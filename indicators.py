...
def analyze_market(df_15m, df_1h, df_4h, df_1d, symbol, funding_rate=None, reasons=None):
    ...
    session_bonus = {
        "LONDON_NY_OVERLAP": 5,
        "ASIA_EUROPE_OVERLAP": 4,
        "LONDON": 3,
        "NEW_YORK": 2,
        "ASIA": 0,
    }.get(sess_ctx.get("name"), 0)
    ...
    session_threshold = {
        "LONDON_NY_OVERLAP": 65.0,
        "ASIA_EUROPE_OVERLAP": 68.0,
        "LONDON": 65.0,
        "NEW_YORK": 72.0,
        "ASIA": 76.0,
    }.get(sess_ctx.get("name"), 999.0)
    ...
    if direction is not None and os.getenv("REQUIRE_FIB_OTE", "1").strip().lower() not in {"0", "false", "no", "off"} and not fib_ote:
    ...
    if daily_bias != market_bias:
        ...
        if abs(total_score) < 78 or not fib_ote or not macd_align:
            return _skip(
    ...
    if smart.get("session", {}).get("name") in NEGATIVE_SESSIONS:
        hli_now = float(smart.get("hyper_liquidity", {}).get("score", 0) or 0)
        macd_align = (direction == "BUY" and macd > macd_signal) or (direction == "SELL" and macd < macd_signal)
