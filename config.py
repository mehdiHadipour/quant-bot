...
# Single manual configuration source shared by live bot and backtest.
_SETTINGS_PATH = Path(__file__).resolve().parent / "manual_settings.json"
try:
    MANUAL_SETTINGS = json.loads(_SETTINGS_PATH.read_text(encoding="utf-8")) if _SETTINGS_PATH.exists() else {}
except (OSError, ValueError, TypeError):
    MANUAL_SETTINGS = {}

def _manual_param(name, default):
    return MANUAL_SETTINGS.get("parameters", {}).get(name, default)

def manual_tool(name):
    return MANUAL_SETTINGS.get("tools", {}).get(name, {})

def gate_enabled(name, default=True):
    return bool(MANUAL_SETTINGS.get("gates", {}).get(name, default))
...
MIN_SIGNAL_PROBABILITY = _bounded_float_env("MIN_SIGNAL_PROBABILITY", float(_manual_param("MIN_SIGNAL_PROBABILITY", 65.0)), 0.0, 100.0)
MIN_ADX = _bounded_float_env("MIN_ADX", float(_manual_param("MIN_ADX", 25.0)), 0.0, 100.0)
ATR_SL_MULTIPLIER = _bounded_float_env("ATR_SL_MULTIPLIER", float(_manual_param("ATR_SL_MULTIPLIER", 1.8)), 0.1, 20.0)
ATR_TP_MULTIPLIER = _bounded_float_env("ATR_TP_MULTIPLIER", float(_manual_param("ATR_TP_MULTIPLIER", 3.0)), 0.1, 50.0)
RISK_PERCENT_PER_TRADE = _bounded_float_env("RISK_PERCENT_PER_TRADE", float(MANUAL_SETTINGS.get("risk",{}).get("RISK_PERCENT_PER_TRADE",1.0)), 0.01, 100.0)
...
MAX_DAILY_LOSS_R = _bounded_float_env("MAX_DAILY_LOSS_R", float(MANUAL_SETTINGS.get("risk",{}).get("MAX_DAILY_LOSS_R",3.0)), 0.0, 100.0)
MAX_OPEN_RISK_R = _bounded_float_env("MAX_OPEN_RISK_R", float(MANUAL_SETTINGS.get("risk",{}).get("MAX_OPEN_RISK_R",4.0)), 0.0, 100.0)
MIN_REWARD_RISK = _bounded_float_env("MIN_REWARD_RISK", float(_manual_param("MIN_REWARD_RISK",1.5)), 0.1, 20.0)
...
STRICT_SYMBOLS = _symbols_env("STRICT_SYMBOLS") or set(MANUAL_SETTINGS.get("strict_symbols", [])) or {
...
STRICT_MIN_SCORE = _bounded_float_env("STRICT_MIN_SCORE", float(_manual_param("STRICT_MIN_SCORE",80.0)), 0.0, 150.0)
STRICT_MIN_HLI = _bounded_float_env("STRICT_MIN_HLI", float(_manual_param("STRICT_MIN_HLI",70.0)), 0.0, 100.0)
STRICT_REQUIRE_OTE = _str_env("STRICT_REQUIRE_OTE", "1" if gate_enabled("fib_ote", True) else "0")
STRICT_REQUIRE_MACD = _str_env("STRICT_REQUIRE_MACD", "1" if bool(MANUAL_SETTINGS.get("gates",{}).get("strict_macd", True)) else "0")
# Every live/backtest entry must be inside the directional Fibonacci OTE zone.
REQUIRE_FIB_OTE = _str_env("REQUIRE_FIB_OTE", "1" if gate_enabled("fib_ote", True) else "0")
FIB_LOOKBACK = _positive_int_env("FIB_LOOKBACK", int(_manual_param("FIB_LOOKBACK",72)), minimum=20)
FIB_OTE_LOW = _bounded_float_env("FIB_OTE_LOW", float(_manual_param("FIB_OTE_LOW",0.618)), 0.50, 0.90)
FIB_OTE_HIGH = _bounded_float_env("FIB_OTE_HIGH", float(_manual_param("FIB_OTE_HIGH",0.786)), 0.60, 0.95)
COUNTERTREND_MIN_SCORE = _bounded_float_env("COUNTERTREND_MIN_SCORE", float(_manual_param("COUNTERTREND_MIN_SCORE",82.0)), 0.0, 150.0)
NEGATIVE_SESSIONS = _symbols_env("NEGATIVE_SESSIONS") or set(MANUAL_SETTINGS.get("sessions",{}).get("negative_sessions", ["ASIA","NEW_YORK"]))
NEGATIVE_SESSION_MIN_SCORE = _bounded_float_env("NEGATIVE_SESSION_MIN_SCORE", float(_manual_param("NEGATIVE_SESSION_MIN_SCORE",80.0)), 0.0, 150.0)
NEGATIVE_SESSION_MIN_HLI = _bounded_float_env("NEGATIVE_SESSION_MIN_HLI", float(_manual_param("NEGATIVE_SESSION_MIN_HLI",70.0)), 0.0, 100.0)
...
# V30.9.7: fully configurable regime/session research controls.
REGIME_CFG = MANUAL_SETTINGS.get("regime", {})
SESSION_CFG = MANUAL_SETTINGS.get("sessions", {})
def manual_bool(section, name, default=False):
    v = MANUAL_SETTINGS.get(section, {}).get(name, default)
    return v if isinstance(v, bool) else str(v).strip().lower() not in {"0","false","no","off"}
def manual_number(section, name, default):
    try: return float(MANUAL_SETTINGS.get(section, {}).get(name, default))
