...
# v25.12: second trailing stage. Once price is this fraction of the way
# from entry to TP, SL moves to lock in PARTIAL_LOCK_R worth of REAL
# profit (in units of the trade's original risk) instead of just
# breakeven — so a trade that gets most of the way to target and then
# reverses still banks a partial win instead of round-tripping all the
# way back to 0R. Must be reached after TRAILING_TRIGGER_R (checked
# first in check_trailing_stop so a candle that jumps straight past both
# thresholds locks the partial profit directly, skipping breakeven).
PARTIAL_LOCK_TRIGGER_R = _float_env("PARTIAL_LOCK_TRIGGER_R", 0.75)
PARTIAL_LOCK_R = _float_env("PARTIAL_LOCK_R", 0.5)
...
# v25.12: default changed from 5 to 10 minutes. Two independent reasons:
# (1) the strategy itself is built on 1h/4h/1d candles — 5 vs 10 minute
# polling makes no real difference to signal quality. (2) the silence
# watchdog's alert threshold is CYCLE_MINUTES × SILENCE_GAP_MULTIPLIER,
# so doubling this doubles the slack (30min -> 60min) against GitHub's
# own documented cron scheduling jitter under load, meaningfully cutting
# false "silence" alerts without weakening real-outage detection.
CYCLE_MINUTES = int(_float_env("CYCLE_MINUTES", 10))
